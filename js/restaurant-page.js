...
function getScenicDetail() {
  ...
  axios.get(`https://tdx.transportdata.tw/api/basic/v2/Tourism/Restaurant?%24filter=contains%28RestaurantID%2C%27${id}%27%29&%24top=30&%24format=JSON
    ...
    .then((res) => {
      ...
      document.querySelector('#scenic-title').textContent = thisData.RestaurantName;
      ...
      if (thisData.WebsiteUrl !== undefined) {
        website.innerHTML = `<span id="scenic-website"><a href='${thisData.WebsiteUrl}' target='_blank'>${thisData.RestaurantName}</a></span>`
      } else {
...
function scenicSpotRecommend() {
  let scenicSpot_url = 'https://tdx.transportdata.tw/api/basic/v2/Tourism/Restaurant?%24top=4&%24format=JSON';
